...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_  HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	//ウィンドウのサイズを修正
	RECT rc1, rc2;
	GetWindowRect(hWnd, &rc1);//ウィンドウの
	GetClientRect(hWnd, &rc2);//
	int sx = SCREEN_WIDTH;
	int sy = SCREEN_HEIGHT;
	sx += ((rc1.right - rc1.left) - (rc2.right - rc2.left));
	sy += ((rc1.bottom - rc1.top) - (rc2.bottom - rc2.top));
	SetWindowPos(hWnd, NULL,0,0,sx,sy,(SWP_NOZORDER | SWP_NOOWNERZORDER | SWP_NOMOVE));//ウィンドウのサイズ変更
	...
	long long frequency; //どのくらい細かく時間をカウントできるか
	...
	while(1)
	{
		...
		else
		{
			...
			nowTick = GetTickCount64(); //現在の時間を取得
			//前回計測から1000ミリ秒が経過したか？
